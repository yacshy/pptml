<template>
  <i clip-coat class="absolute clip right" @mousedown.prevent="mouseDownHandle"></i>
</template>
...
@Component
export default class RightClip extends Vue {
  ...
  mouseDownHandle(e: MouseEvent): void {
    ...
    if (this.shape === 'polygon') {
      this.polygon = this.style
        .slice(8, -1)
        .split(', ')
        .map(v => v.split(' ').map(i => parseFloat(i)))
      this.max = Math.max(...this.polygon.map(i => i[0]))
      this.min = Math.min(...this.polygon.map(i => i[0]))
  ...
  mouseMoveHandle(e: MouseEvent): void {
    this.offset = (100 * (e.screenX - this.start)) / this.active.width
    if (this.shape === 'rect') {
      const [T, R, B, L] = this.rectSize
      this.result = `inset(${T}% ${R - this.offset}% ${B}% ${L}% round ${
        this.radius
      })`
    } else if (this.shape === 'polygon') {
      const str = this.polygon
        .map(v => [
          v[0] + (this.offset * (v[0] - this.min)) / (this.max - this.min),
          v[1]
      ...
      this.result = `ellipse(${w + this.offset / 2}% ${h}% at ${x +
        this.offset / 2}% ${y}%)`
    }
