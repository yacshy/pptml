<template>
  <i clip-coat class="absolute clip top" @mousedown.prevent="mouseDownHandle"></i>
</template>
...
@Component
export default class TopClip extends Vue {
  ...
  mouseMoveHandle(e: MouseEvent): void {
    this.offset = ((e.screenY - this.start) * 100) / this.active.height
    if (this.shape === 'polygon') {
      const str = this.polygon
        .map(v => [
          v[0],
          v[1] + (this.offset * (this.max - v[1])) / (this.max - this.min)
      ...
      this.result = `inset(${T + this.offset}% ${R}% ${B}% ${L}% round ${
        this.radius
      })`
    } else if (this.shape === 'ellipse') {
